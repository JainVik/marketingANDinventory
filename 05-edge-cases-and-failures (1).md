...
## 12. Identity, devices & browse tracking (items 67, 68)

**Requirements list for `modules/identity` and `modules/telemetry`.** `docs/03` §0 (PII boundary), §2.10, §3.4; `docs/02` §4b.

### 12.1 One identity, everywhere

1. **Same phone, second outlet** → one `pii.customers` row, a second `customer_profiles` row. The customer's cross-outlet history (#30) is served from `pii` by the identity service; the second vendor sees nothing of the first. 🧪
2. **Returning device, new outlet** → `POST /auth/customer/resume` succeeds from the refresh cookie, no OTP, and the menu renders already identified. If the cookie is missing or expired, the scan falls back to anonymous browsing and OTP only at checkout — never a hard block. 🧪
3. **Refresh-token reuse detected** (a rotated token replayed) → revoke the entire `family_id`, force OTP, write an `audit_logs` row. A stolen cookie survives exactly one use. 🧪
4. **Same phone on a second device** → both devices stay signed in, each with its own `customer_devices` row and token family; `/me/devices` lists them and either can be revoked. No device limit in v1.
5. **Shared/borrowed phone** (a friend orders on someone else's device) → OTP for a *different* phone on a device that already has a session creates a new identity and a new device row; the old family is revoked on that device. The device follows the last verified human. 🧪
6. **180-day expiry mid-visit** → the access token is refreshed transparently; if the refresh itself is expired, the customer is dropped to anonymous and re-OTPs at checkout. Their cart survives (localStorage), the browse session survives, `visitors.customer_id` is re-stitched on the new verify. 🧪
7. **`pii` unreachable / identity pool exhausted** → ordering degrades but does not stop: anonymous browsing, cart and staff-entered orders keep working; OTP and reveal return 503. The order desk keeps running. 🧪

### 12.2 Reveal on first order

8. **Vendor asks for a name before the first order** → `pii.reveal_identity()` raises `42501` → 403 `CUSTOMER_NOT_REVEALED`. This is tested per-vendor, not just per-role: vendor A having revealed customer X must not let vendor B read X. 🧪
9. **First order placed** → `revealed_at`, `display_name` and the `customer_revealed_to_vendor` event are written **in the same transaction** as `order_placed`. A rolled-back order reveals nobody. 🧪
10. **Customer renames themselves afterwards** → `pii.customers.name` changes; `display_name` on already-revealed profiles is a snapshot and does **not** retroactively change (same rule as `order_lines` price snapshots). The next reveal call returns the current name.
11. **Staff-entered walk-in with a phone typed at the counter** → counts as an order at that outlet, so it reveals normally. A phone typed with **no** order does not reveal, and does not create a profile. 🧪
12. **Imported CSV list** → `lifecycle='imported'`, `revealed_at` set (the vendor supplied these people themselves — it is their own list) but `consent_marketing=false`; §5.2 still blocks marketing until an opt-in exists. 🧪
13. **Erasure while revealed** → `display_name` and `phone_hash` null out, aggregates and `revealed_at` stay, so the vendor's counts and revenue history survive an erasure. 🧪

### 12.3 Browse tracking

14. **Telemetry failure of any kind** (validation, rate limit, DB error, `pii` down) → log, drop, respond 202. It must be impossible for a browse write to fail an order, slow a placement, or surface in the customer's console. 🧪
15. **Beacon retried after network loss** → `(browse_session_id, seq)` unique index makes the replay a no-op; duplicate flushes never inflate view counts. 🧪
16. **Client sends a `visitorId` that does not exist** (cleared storage, forged id) → issue a fresh one and use it; never trust a client-supplied id as proof of anything. Browse rows are attributable, not authenticated.
17. **`browse_session` never ends** (tab closed without an unload beacon) → the sweeper closes sessions idle for 30 minutes, sets `ended_at`, computes `dwell_ms` from `last_seen_at`, and stamps `outcome`. 🧪
18. **OTP mid-visit** → `visitors.customer_id` is set and `browse_sessions.customer_id` is backfilled for that visitor's **last 24 hours only**; older anonymous history stays anonymous. One `visitor_identified` event. 🧪
19. **Practice table** → `is_practice` copied from the table; practice browsing never reaches an owner's funnel, insights or ML export. 🧪
20. **Owner asks who looked and left** → aggregates only. `/vendor/insights/funnel` returns counts; no endpoint returns a person with `revealed_at IS NULL`. A cafe learns "37 scanned, 22 opened the pizza, 9 ordered", never "Rahul looked and left". 🧪
21. **Volume spike / bot traffic on a QR** → per-outlet cap of 240 flushes/min; beyond it the beacon drops silently and an ops alert fires. `browse_events` is hard-capped at 200 rows per session.
22. **Unlinked visitor retention** → visitors that never link to a customer are deleted after `purge_after` (400 days) along with their browse rows; linked ones follow the customer's own erasure path. 🧪
