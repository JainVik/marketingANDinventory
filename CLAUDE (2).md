...
**Read receipt — before writing any code in a session, post this and wait:**

1. The doc sections you actually read for this module (from its card in `docs/16` §3).
2. The constraints you are working under, in your own words — not quoted back, restated.
3. The `docs/05` cases for this module, listed, with the 🧪 ones marked.
4. Anything on the Reads list you did **not** read, and anything you cannot satisfy as written.

Point 4 is not optional and "none" is a real answer only if it is true. A session that starts coding without this is a session nobody can review.
...
5b. **The PII wall (items 67, 68).** Customer identity lives in schema `pii` and `regulars_app` has **no grant** on it. Never add one. Outside `modules/identity/` you may not import the `regulars_identity` pool, query `pii.*` directly, or store a phone number or a customer's real name in an `app` table. To show a name or phone, call `pii.reveal_identity(customerId)` — it refuses (403 `CUSTOMER_NOT_REVEALED`) unless that customer has ordered with the calling vendor, and that refusal is the product rule, not an obstacle to route around. Match customers on `phone_hash`, never on the phone. The two documented exceptions (`bills.customer_phone`, the WhatsApp ledger's `phone`) are in `docs/03` §0 — do not create a third.
5c. **Reveal is transactional.** `customer_profiles.revealed_at` and `display_name` are written in the **same transaction** as that customer's first `order_placed` at that vendor. Not in a job, not in a webhook, not "on next login".
...
9b. **Browse telemetry is not the outbox.** `visitors`, `browse_sessions` and `browse_events` are written by `modules/telemetry` on their own path, outside any order transaction, and never through `app.events`. A telemetry write that is slow, invalid, rate-limited or failing must be logged and dropped — the endpoint always answers 202. If a telemetry call can fail a customer's order, that is a defect, not a trade-off.
...
- [ ] New table in `pii`, or any new grant → PII-wall gate re-run (`docs/06 §4b`); no new door to identity without an entry in that list
